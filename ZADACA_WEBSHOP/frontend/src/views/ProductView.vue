<template>
    <div v-if="proizvod">
      ...
      <div class="mt-4">
        <label for="quantity" class="text-sm font-medium text-gray-700">Količina:</label>
        <input type="number" id="quantity" v-model="narucena_kolicina" min="1" class="mt-2 w-16 p-2 border border-gray-300 rounded-md" />
      </div>
...
const narucena_kolicina = ref(1); 

const dodaj_kosarica = async () => {
  if (odabrana_velicina.value && odabrana_boja.value && narucena_kolicina.value > 0) {
    const podaci = {
      id: proizvod.value.id,
      velicina: odabrana_velicina.value,
      boja: odabrana_boja.value,
      narucena_kolicina: narucena_kolicina.value
    };
    ...
    console.log('Morate odabrati veličinu, boju i količinu.');
  }
};
